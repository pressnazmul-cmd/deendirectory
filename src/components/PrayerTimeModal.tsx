...
const PrayerTimeModal = ({ open, onOpenChange, instituteName, address, instituteId }: PrayerTimeModalProps) => {
  const [times, setTimes] = useState<PrayerTimes | null>(null);
  const [loading, setLoading] = useState(false);
  const [dateStr, setDateStr] = useState("");

  // Fetch manual prayer times from DB
  const { data: manualTimes } = useQuery({
    queryKey: ["prayer-times", instituteId],
    queryFn: async () => {
  ...
  useEffect(() => {
    ...
    // If manual times exist and have values, use them
    if (manualTimes && manualTimes.fajr) {
      setTimes({
        Fajr: manualTimes.fajr,
        Dhuhr: manualTimes.dhuhr,
        Asr: manualTimes.asr,
        Maghrib: manualTimes.maghrib,
        Isha: manualTimes.isha,
      });
      setLoading(false);
      return;
    }
        ...
        <div className="bg-gradient-to-br from-zinc-800 to-zinc-900 px-5 pt-5 pb-4 text-white">
          <div className="flex items-start justify-between">
            <DialogHeader className="space-y-0.5">
              <DialogTitle className="text-lg font-bold text-white leading-tight">PRAYER TIMES</DialogTitle>
            </DialogHeader>
        ...
        <div className="px-4 py-3 space-y-2">
          {loading ? (
            ...
            Object.entries(times).map(([name, time]) => {
              ...
              return (
                <div key={name} className="flex items-center justify-between rounded-lg bg-zinc-900 px-4 py-3 text-white">
